...
def render_synced_game():
    ...
    if my_team_index is not None:
        ...
        st.caption("3 zsetonért kérhettek 1 véletlen évszámkártyát.")
        ...
        if team_tokens(my_team_for_exchange) >= 3:
            if st.button(
                ...
                latest = get_shared_game(st.session_state.game_code)
                latest_teams = normalized_teams(latest["teams"])
                latest_remaining = list(latest.get("remaining_songs") or [])
                ...
                if (
                    ...
                    latest_teams[my_team_index]["timeline"].append(bonus_song)
